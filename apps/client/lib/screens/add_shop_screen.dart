...
import 'package:wonwon_client/screens/map_picker_screen.dart';
...
class _AddShopScreenState extends State<AddShopScreen> {
  ...
  // Note: address is constructed from building/soi/district/province fields
  // area is derived from district field
  ...
  void dispose() {
    ...
    // _addressController and _areaController removed (unused in form)
                          ...
                          child: Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  'subservice_${subService.categoryId}_${subService.id}'.tr(context),
                                  style: TextStyle(
                                    color:
                                        isSelected
                                            ? Colors.white
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      // Validate coordinates are within Thailand (approx bounds)
      if (lat < 5.5 || lat > 20.5 || lng < 97.0 || lng > 106.0) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('coordinates_outside_thailand'.tr(context)),
            backgroundColor: Colors.orange,
          ),
        );
        return;
      ...
      // Validate opening hours: at least one day must be open
      final hasOpenDay = _closedDays.values.any((closed) => !closed);
      if (!hasOpenDay && !_hasIrregularHours) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('at_least_one_day_open'.tr(context)),
            backgroundColor: Colors.orange,
          ),
        );
        return;
      ...
      if (_selectedImageBytes != null) {
        ...
        if (imageUrl != null) {
          ...
          // Notify user that image upload failed but continue with submission
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
    ...
    try {
      ...
      if (mounted) {
        ...
        if (success) {
          ...
          AnalyticsService.safeLog(() => AnalyticsService().logAddShop(shopId: shop.id, shopName: shop.name));
          _showSuccessDialog();
        } else {
          // Show error dialog
          _showErrorDialog();
