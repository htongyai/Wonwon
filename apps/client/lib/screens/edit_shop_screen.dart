...
class _EditShopScreenState extends State<EditShopScreen> {
          ...
          Wrap(
            ...
            children:
                _availableCategories.map((category) {
                  ...
                  return FilterChip(
                    label: Text('category_$category'.tr(context)),
                    selected: isSelected,
                    onSelected: (selected) {
                      setState(() {
                        if (selected) {
      ...
      child: Column(
        ...
        children: [
          ...
          ..._availablePaymentMethods.map((method) {
            final labelKey = 'payment_$method';
  ...
  Future<void> _saveShop() async {
    ...
    try {
      ...
      for (int i = 0; i < dayDisplayNames.length; i++) {
        final dayName = dayDisplayNames[i];
        final key = shortKeys[i];
      ...
      if (mounted) {
        ...
        AnalyticsService.safeLog(() => AnalyticsService().logEditShop(widget.shop.id));
