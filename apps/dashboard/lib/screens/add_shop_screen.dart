...
class _AddShopScreenState extends State<AddShopScreen> {
            ...
            child: SingleChildScrollView(
              ...
              child: Padding(
                ...
                child: Form(
                  ...
                  // Google Maps Link Paste Section
                  _buildGoogleMapsLinkSection(),
                  SizedBox(height: ResponsiveSize.getHeight(4)),
                  ...
                  _buildTextFormField(
                    ...
                    label: '${'phone_label'.tr(context)} (${'optional'.tr(context)})',
                    hint: 'enter_phone_hint'.tr(context),
                  ...
                  Wrap(
                    ...
                    children:
                        {
                              'cash': 'payment_cash'.tr(context),
                              'card': 'payment_card'.tr(context),
                              'qr': 'payment_qr'.tr(context),
                              'bank_transfer': 'payment_bank_transfer'.tr(context),
                            }
                            .entries
                            .map(
                              (entry) => FilterChip(
                                ...
                                onSelected: (selected) {
                                  setState(() {
                                    if (selected) {
                                      _selectedPaymentMethods.add(entry.key);
                                    } else {
                                      _selectedPaymentMethods.remove(entry.key);
                                    }
                                  });
                                },
                              ),
  ...
  Widget _buildGoogleMapsLinkSection() {
    return Container(
          ...
          Row(
            ...
            children: [
              ...
              SizedBox(
                height: 48,
                child: ElevatedButton.icon(
                  onPressed: _isExtractingLink ||
                          _googleMapsLinkController.text.trim().isEmpty
                      ? null
                      : _extractFromGoogleMapsLink,
                  icon: _isExtractingLink
                      ? const SizedBox(
                          width: 16,
                          height: 16,
                          child: CircularProgressIndicator(
  ...
  Future<void> _extractFromGoogleMapsLink() async {
    final url = _googleMapsLinkController.text.trim();
    ...
    if (!GoogleMapsLinkService.isGoogleMapsUrl(url)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('invalid_google_maps_link'.tr(context)),
          backgroundColor: Colors.orange,
        ),
      );
      return;
    ...
    try {
      ...
      setState(() {
        ...
        // Phone
        if (result.phoneNumber != null && _phoneController.text.isEmpty) {
        ...
        if (result.buildingNumber != null &&
          ...
          _buildingNumberController.text = result.buildingNumber!;
          fieldsExtracted++;
        }

        // Street / Soi
        if (result.street != null && _soiController.text.isEmpty) {
          _soiController.text = result.street!;
          fieldsExtracted++;
        }

        // District
        if (result.district != null && _districtController.text.isEmpty) {
          _districtController.text = result.district!;
          fieldsExtracted++;
        }

        // Landmark
        if (result.landmark != null && _landmarkController.text.isEmpty) {
          _landmarkController.text = result.landmark!;
          fieldsExtracted++;
        }

        // Province
        if (result.province != null && _selectedProvince == null) {
          final matchedProvince = _matchProvince(result.province!);
        ...
        // Categories
        if (result.matchedCategories != null &&
            result.matchedCategories!.isNotEmpty &&
            _selectedCategories.isEmpty) {
          for (final cat in result.matchedCategories!) {
          ...
          if (_selectedCategories.isNotEmpty) fieldsExtracted++;
        }

        // Website → other contacts
        if (result.website != null && _otherContactsController.text.isEmpty) {
          _otherContactsController.text = result.website!;
          fieldsExtracted++;
        }

        // Opening hours
        if (result.openingHours != null && result.openingHours!.isNotEmpty) {
          _applyExtractedHours(result.openingHours!, context);
          fieldsExtracted++;
        }
      ...
      if (result.photoUrls != null &&
          result.photoUrls!.isNotEmpty &&
          _selectedImageBytes == null) {
        _downloadPhotoFromUrl(result.photoUrls!.first).then((bytes) {
      ...
      if (!mounted) return;
      _showExtractionSuccessDialog(result, fieldsExtracted);
    } catch (e) {
      if (!mounted) return;
      setState(() {
        _isExtractingLink = false;
  ...
  Future<Uint8List?> _downloadPhotoFromUrl(String url) async {
    try {
      ...
      appLog('Failed to download photo from URL: $e');
    }
    return null;
  }

  void _applyExtractedHours(Map<int, PlaceOpeningPeriod> hours, BuildContext ctx) {
    const dayMapping = {
    ...
    for (final entry in hours.entries) {
      final dayName = dayMapping[entry.key];
      if (dayName == null) continue;

      final period = entry.value;
      if (period.isClosed) {
        _closedDays[dayName] = true;
        _openingTimeControllers[dayName]?.clear();
        _closingTimeControllers[dayName]?.clear();
        _openingTimes[dayName] = null;
        _closingTimes[dayName] = null;
      } else {
        _closedDays[dayName] = false;
        ...
        _openingTimes[dayName] = openTime;
        _closingTimes[dayName] = closeTime;
        _openingTimeControllers[dayName]?.text = openTime.format(ctx);
        _closingTimeControllers[dayName]?.text = closeTime.format(ctx);
      }
    }
  }

  void _showExtractionSuccessDialog(GoogleMapsLinkResult result, int count) {
    final items = <_ExtractedItem>[];

    items.add(_ExtractedItem(Icons.location_on, 'extracted_coordinates'.tr(context),
        '${result.latitude.toStringAsFixed(4)}, ${result.longitude.toStringAsFixed(4)}'));
    if (result.placeName != null) {
      items.add(_ExtractedItem(Icons.store, 'extracted_shop_name'.tr(context), result.placeName!));
    }
    if (result.phoneNumber != null) {
      items.add(_ExtractedItem(Icons.phone, 'extracted_phone'.tr(context), result.phoneNumber!));
    }
    if (result.street != null) {
      items.add(_ExtractedItem(Icons.signpost, 'extracted_street'.tr(context), result.street!));
    }
    if (result.buildingNumber != null) {
      items.add(_ExtractedItem(Icons.home, 'extracted_building'.tr(context), result.buildingNumber!));
    }
    if (result.district != null) {
      items.add(_ExtractedItem(Icons.map, 'extracted_district'.tr(context), result.district!));
    }
    if (result.province != null) {
      items.add(_ExtractedItem(Icons.flag, 'extracted_province'.tr(context), result.province!));
    }
    if (result.openingHours != null && result.openingHours!.isNotEmpty) {
      items.add(_ExtractedItem(Icons.access_time, 'extracted_hours'.tr(context),
    ...
    if (result.matchedCategories != null && result.matchedCategories!.isNotEmpty) {
      items.add(_ExtractedItem(Icons.category, 'extracted_categories'.tr(context),
          result.matchedCategories!.join(', ')));
    }
    if (result.website != null) {
      items.add(_ExtractedItem(Icons.language, 'extracted_website'.tr(context), result.website!));
    }
    if (result.landmark != null) {
      items.add(_ExtractedItem(Icons.place, 'extracted_landmark'.tr(context), result.landmark!));
    }
    if (result.photoUrls != null && result.photoUrls!.isNotEmpty) {
      items.add(_ExtractedItem(Icons.photo_camera, 'extracted_photo'.tr(context),
          'downloading_photo'.tr(context)));
    }
          ...
          child: Column(
            ...
            children: [
              ...
              ...items.map((item) => Padding(
                    ...
                    child: Row(
                      children: [
                        Icon(item.icon, size: 18, color: AppConstants.primaryColor),
                        const SizedBox(width: 8),
                        Text('${item.label}: ',
  ...
  String? _matchProvince(String province) {
    ...
    // Partial match
    for (final p in _provinces) {
      if (p.toLowerCase().contains(normalized) ||
          normalized.contains(p.toLowerCase())) {
        return p;
      }
    }

    // Common Thai-to-English province mappings
    const thaiToEnglish = {
      'กรุงเทพ': 'Bangkok',
    ...
    for (final entry in thaiToEnglish.entries) {
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    Text(
                      '${_getCategoryDisplayName(category)} ${'services_label'.tr(context)}',
                      style: const TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.w600,
                      ),
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      // Create a map of opening hours using short keys (mon, tue, etc.)
      const dayToShortKey = {
        'Monday': 'mon',
        'Tuesday': 'tue',
        'Wednesday': 'wed',
      ...
      _openingTimeControllers.forEach((day, openingController) {
        final closingController = _closingTimeControllers[day]!;
        final isClosed = _closedDays[day] ?? false;
        final key = dayToShortKey[day] ?? day.toLowerCase();
        ...
        if (isClosed) {
          ...
          closingDays.add(day);
            ...
            closingController.text.isNotEmpty) {
          hours[key] = '${openingController.text} - ${closingController.text}';
        }
      });
      ...
      if (_selectedImageBytes != null) {
        ...
        if (imageUrl != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('image_upload_failed_continuing'.tr(context)),
              backgroundColor: Colors.orange,
  ...
  Future<void> _pickAndCropImage() async {
    try {
      ...
      final croppedFile = await ImageCropper().cropImage(
        ...
        uiSettings: [
          ...
          IOSUiSettings(
            ...
            doneButtonTitle: 'done'.tr(context),
            cancelButtonTitle: 'cancel'.tr(context),
          ),
        ],
      );
      ...
      if (!mounted) return;
      setState(() {
        _imageError = 'image_process_error'.tr(context);
        _selectedImageBytes = null;
      });
  ...
  Future<void> _compressAndSetImage(Uint8List imageBytes) async {
    try {
      ...
      if (compressedBytes.length > 100 * 1024) {
        if (!mounted) return;
        setState(() {
          _imageError = 'image_too_large'.tr(context);
          _selectedImageBytes = null;
        });
        return;
      }
...
/// Helper class for the extraction success dialog
class _ExtractedItem {
  final IconData icon;
  final String label;
  final String value;
  const _ExtractedItem(this.icon, this.label, this.value);
}
