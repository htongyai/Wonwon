...
import 'package:wonwon_dashboard/widgets/admin/analytics/registration_trend_chart.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/category_chart.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/approval_status_chart.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/engagement_chart.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/rating_distribution_chart.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/area_distribution_chart.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/metric_card.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/top_shops_section.dart';
import 'package:wonwon_dashboard/widgets/admin/analytics/category_performance_section.dart';
...
class _AdminAnalyticsScreenState
  ...
  Future<Map<String, dynamic>> _fetchAnalyticsData() async {
    ...
    // Fetch reviews from subcollections: shops/{shopId}/review
    final reviewDocs = <Map<String, dynamic>>[];
    ...
    for (final shopDoc in shopsSnapshot.docs) {
      reviewFutures.add(
        _firestore.collection('shops').doc(shopDoc.id).collection('review').get()
            ...
            .catchError((_) => null as QuerySnapshot?),
      );
    }
    final reviewSnapshots = await Future.wait(reviewFutures, eagerError: false);
    for (int i = 0; i < reviewSnapshots.length; i++) {
      final snapshot = reviewSnapshots[i];
      if (snapshot == null) continue;
      final shopId = shopsSnapshot.docs[i].id;
      for (final doc in snapshot.docs) {
        final data = doc.data();
        if (data is Map<String, dynamic>) {
          reviewDocs.add({...data, 'shopId': data['shopId'] ?? shopId});
        }
      }
    }
  ...
  Widget _buildPerformanceInsights() {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final children = [
          isWide ? Expanded(child: TopShopsSection(topShops: topShops, onShopTap: _showShopDetails))
                 : TopShopsSection(topShops: topShops, onShopTap: _showShopDetails),
          SizedBox(width: isWide ? 16 : 0, height: isWide ? 0 : 16),
          isWide ? Expanded(child: CategoryPerformanceSection(categoryPerformance: categoryPerformance, onCategoryTap: _showCategoryPerformanceDetails))
                 : CategoryPerformanceSection(categoryPerformance: categoryPerformance, onCategoryTap: _showCategoryPerformanceDetails),
        ];
      ...
      children: [
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            if (isWide) {
              return Row(children: [
                Expanded(child: ShopCategoryChart(data: _analyticsData['shopsByCategory'] as Map<String, int>? ?? {}, onCategoryTap: _showCategoryDetails)),
                const SizedBox(width: 16),
                Expanded(child: ApprovalStatusChart(approvedShops: _analyticsData['approvedShops'] ?? 0, pendingShops: _analyticsData['pendingShops'] ?? 0)),
              ]);
            }
            return Column(children: [
              ShopCategoryChart(data: _analyticsData['shopsByCategory'] as Map<String, int>? ?? {}, onCategoryTap: _showCategoryDetails),
              const SizedBox(height: 16),
              ApprovalStatusChart(approvedShops: _analyticsData['approvedShops'] ?? 0, pendingShops: _analyticsData['pendingShops'] ?? 0),
            ]);
          },
        ),
        const SizedBox(height: 24),
        ...
        LayoutBuilder(
          builder: (context, constraints) {
            ...
            if (isWide) {
              return Row(children: [
                Expanded(child: UserEngagementChart(activeUsers: _analyticsData['activeUsers'] ?? 0, totalUsers: _analyticsData['totalUsers'] ?? 0)),
                const SizedBox(width: 16),
                Expanded(child: RatingDistributionChart(data: _analyticsData['ratingDistribution'] as Map<int, int>? ?? {})),
              ]);
            }
            return Column(children: [
              UserEngagementChart(activeUsers: _analyticsData['activeUsers'] ?? 0, totalUsers: _analyticsData['totalUsers'] ?? 0),
              const SizedBox(height: 16),
              RatingDistributionChart(data: _analyticsData['ratingDistribution'] as Map<int, int>? ?? {}),
            ]);
          },
        ),
        const SizedBox(height: 24),
  ...
  Widget _buildUserActivitySection() {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (isWide) {
          return Row(children: [
            Expanded(child: _buildRecentUserActivity(recentUsers.take(10).toList())),
            const SizedBox(width: 16),
            Expanded(child: _buildUserEngagementStats()),
          ]);
        }
        return Column(children: [
          _buildRecentUserActivity(recentUsers.take(10).toList()),
          const SizedBox(height: 16),
