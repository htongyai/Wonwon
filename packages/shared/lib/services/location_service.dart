  ...
  Future<Position?> getCurrentPosition() async {
    ...
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        appLog('Location permission denied forever');
        _permissionStatus = LocationPermissionStatus.permanentlyDenied;
        notifyListeners();
        return null;
      }
      ...
      if (kIsWeb) {
        ...
        appLog('Web location: Starting geolocation attempts...');
        appLog('Web location: User agent: ${kIsWeb ? "Web" : "Mobile"}');
        appLog('Web location: Current URL: ${Uri.base}');
        
        try {
          // First attempt: High accuracy with timeout
          appLog('Web location: Attempting high accuracy...');
          ...
          appLog('High accuracy failed: $e1');

          try {
            // Second attempt: Medium accuracy with shorter timeout
            appLog('Retrying with medium accuracy...');
            final position = await Geolocator.getCurrentPosition(
            ...
            appLog('Medium accuracy failed: $e2');

            try {
              // Third attempt: Low accuracy with very short timeout
              appLog('Retrying with low accuracy...');
              final position = await Geolocator.getCurrentPosition(
              ...
              appLog('All web attempts failed: $e3');
        ...
        appLog('Got position: ${position.latitude}, ${position.longitude}');
        _currentPosition = position;
        notifyListeners();
        return position;
      }
    } catch (e) {
      appLog('Error getting current position: $e');
      _lastFailedAttempt = DateTime.now();
      return null;
    }
  }
