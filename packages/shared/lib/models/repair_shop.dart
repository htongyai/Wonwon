class RepairShop {
  ...
  final String? rejectionReason; // Reason for rejection (if rejected by admin)
  final String approvalStatus; // 'pending', 'approved', 'rejected'

  // Per-shop analytics lifetime totals
  ...
  factory RepairShop.fromMap(Map<String, dynamic> map) {
    ...
    return RepairShop(
      ...
      approvalStatus: map['approvalStatus']?.toString() ??
          (map['approved'] == true ? 'approved' : 'pending'),
      totalViews: (map['totalViews'] as num?)?.toInt() ?? 0,
      totalSaves: (map['totalSaves'] as num?)?.toInt() ?? 0,
